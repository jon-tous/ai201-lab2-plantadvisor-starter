...
SYSTEM_PROMPT = (
    ...
    "If you cannot find the plant, acknowledge the limitation and give the best "
    "general care advice you can for the plant type or symptoms described.\n\n"
...
def run_agent(user_message: str, history: list) -> str:
    ...
    The agent loop follows a specific pattern that you'll implement here.
    """
    ...
    while round_count < MAX_TOOL_ROUNDS:
        try:
            ...
            print(f"run_agent error: {exc}")
            return (
                "Sorry, I had trouble contacting the plant advisor service. "
                "Please try again in a moment."
            )

        if not getattr(response, "choices", None):
            return (
                "Sorry, I couldn't get a valid response from the plant advisor. "
                "Please try again."
            )
        ...
        for tool_call in assistant_message.tool_calls:
            try:
                ...
                print(f"tool call parse error: {exc}")
                return (
                    "Sorry, I couldn't process a tool request. "
    ...
    final_text = getattr(last_assistant_message, "content", None)
    if not final_text:
        return "Sorry, I couldn't produce a response. Please try again."
